...
- la qualification optionnelle passe 38 tests avec 2 skips PIL ; Docling, RapidOCR, ONNX Runtime, sentence-transformers et PyMuPDF sont absents de `.venv`, donc le smoke advanced est `NOT_CONFIGURED` et aucune activation implicite n’est revendiquée ; détail dans `SMART_AO_PHASE11_LOCAL_OPTIONAL_DCE_LIMITS_v0.1.md` ;
- décision d’activation : OCR/RapidOCR, Docling/PyMuPDF et RAG/sentence-transformers restent différés ; aucun extra ni modèle n’est installé sans corpus approuvé et budget mémoire mesuré ; détail dans `SMART_AO_PHASE11_OPTIONAL_DEPENDENCIES_DECISION_v0.1.md` ;
