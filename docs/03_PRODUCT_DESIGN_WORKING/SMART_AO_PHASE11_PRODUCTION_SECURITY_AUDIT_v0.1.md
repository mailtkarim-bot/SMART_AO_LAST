...
- la qualification optionnelle passe 9 tests avancés sans skip PIL après installation ; PyMuPDF et l’import Docling passent le smoke test, RapidOCR est testable, sentence-transformers et Torch s’importent en CPU ; aucun modèle n’est téléchargé ;
- décision d’activation : les extras sont installés depuis `uv.lock`, mais OCR/Docling/RAG restent runtime désactivés ou `PARTIAL` sans modèles, corpus approuvé et budget mémoire mesuré ; détail dans `SMART_AO_PHASE11_OPTIONAL_DEPENDENCIES_DECISION_v0.1.md` ;
