...
## OWNER_CONSOLIDATED v0.3 — organisation propriétaire de la surface UX complète
...
Source métier : [OWNER_CONSOLIDATED v0.4](../../docs/00_REFERENCE_ACTIVE/SMART_AO_Cahier_Directeur_Produit_Metier_OWNER_FREEZE_v2.0.md). SHA-256 : `af461cf4782af01575e707d97a724ac1c5d1a5c2229b9ec820010c58a4e05042`.
...
Provenance : contre-revue Work `01_CONTRE_REVUE_UX.md`, benchmark `02_BENCHMARK_ET_SOURCES.md`, contrôle `03_CONTROLE_ET_REPRISE.md`. Ces fichiers deviennent des preuves de conception et non des autorités concurrentes.

## 0. Portée et précédence

La présente version **remplace l’organisation de présentation du catalogue v0.1 comme base propriétaire de prototypage**. Elle ne remplace pas le cahier métier v0.4. Les missions, rôles, états obligatoires et niveaux minimaux de fidélité du v0.1 demeurent applicables sauf modification explicite ci-dessous ; aucune réduction de pouvoir de preuve ni extension de droit n’est implicite dans une fusion.

Les 104 identifiants d’origine sont conservés comme contrats de couverture. Le v0.1 compte 83 SCREEN, 10 FLOW, 2 PANEL, 3 STATE, 1 SHELL et 5 RESPONSIVE ; 52 UX-Critical, 43 UX-Primary et 9 UX-Support. Le v0.3 adopte **16 espaces canoniques**, une structure transversale **C00** et **5 contrats transversaux individualisés N01–N05**. Ce sont des unités de conception de natures différentes : ne pas additionner 104 + 16 + 5 pour annoncer un nombre d’écrans.
...
## 0.1 Arbitrages propriétaire v0.3 — fermeture des huit UX-C1

Les huit écarts C1 de la contre-revue Work sont **acceptés et fermés au niveau décision de conception**. Ils devront encore être prouvés visuellement dans les prototypes et recettes ; « fermé » ne signifie donc ni testé utilisateur, ni implémenté, ni homologué.

| ID | Décision propriétaire v0.3 | Statut |
|---|---|---|
| **UX-C1-01 — files concurrentes** | Adopter une vue **À résoudre** dans C05, composée d’objets typés. Une tâche terminée ne clôt jamais automatiquement exigence, contradiction, hypothèse ou risque. | **FERMÉ — adopté** |
| **UX-C1-02 — Remise dispersée** | Regrouper SUB-01–07 dans **C11 Remise**, avec quatre étapes visuelles persistantes : candidate → contrôles/autorisation → dépôt humain externe → preuve/réception. Signature, P5, export, dépôt et reçu restent des actes distincts. | **FERMÉ — adopté** |
| **UX-C1-03 — priorités d’accueil** | Adopter un **Accueil commun composé selon responsabilités** : décisions, contributions et événements séparés ; calculs et compteurs filtrés par droits avant agrégation. | **FERMÉ — adopté** |
| **UX-C1-04 — couvertures ambiguës** | Interdire un indicateur global trompeur. Distinguer réception, lecture, revue d’exigences et couverture aval (prix/réponse), avec dénominateurs visibles et aucun vert global si un B0/B1 applicable subsiste. | **FERMÉ — adopté** |
| **UX-C1-05 — conflit / résultat inconnu** | Adopter **N01** et **N02**. Conserver contributions concurrentes et dernier état confirmé ; ne jamais appliquer « dernier clic gagne » ni supposer un succès avant preuve. | **FERMÉ — adopté** |
| **UX-C1-06 — confidentialité économique** | Les fusions d’interface ne fusionnent jamais les droits. Contrôler l’accès par champ, agrégat, source, génération IA et export ; prototyper explicitement un rôle sans droit économique. | **FERMÉ — adopté** |
| **UX-C1-07 — onboarding trop long** | Identité et MFA restent obligatoires ; la configuration métier devient progressive. La première valeur doit pouvoir arriver depuis une première Affaire sans exiger une Mémoire Entreprise complète. | **FERMÉ — adopté** |
| **UX-C1-08 — erreurs trop tardives** | Remplacer les vagues purement par domaine par des **parcours complets**, intégrant droits, panne, reprise, mobile utile et mode dégradé dès que la fonction apparaît. | **FERMÉ — adopté** |

## 0.2 Doctrine propriétaire issue de la contre-revue

1. Les **104 IDs v0.1 restent les contrats de couverture** : aucun n’est supprimé comme besoin parce qu’une page est fusionnée.
2. Les **16 espaces C01–C16 sont les espaces canoniques de conception** ; ils ne constituent ni seize routes obligatoires ni seize prototypes suffisants.
3. **C00** s’applique transversalement à toutes les surfaces.
4. **N01–N05** deviennent des contrats UX normatifs pour les prototypes.
5. **PUX-19** devient le parcours bout-en-bout officiel du pilier Entreprise.
6. Les 5 espaces de premier niveau restent : **Accueil / Opportunités / Affaires / Entreprise / Administration**.
7. Dans une Affaire, les cinq destinations quotidiennes sont : **Vue d’ensemble / Documents / Prix / Réponse / Remise** ; Décision, Partenaires et Résultat/Passation restent des destinations contextuelles explicitement retrouvables.
8. Les fusions retirent des **pages autonomes**, jamais les preuves, responsabilités, états, droits ou recettes associés.
9. Toute surface ou compteur respecte les permissions avant agrégation : aucune fuite de marge, de dossier restreint ou de donnée Direction par résumé, recherche, notification ou IA.
10. Aucune innovation UX n’autorise un dépôt autonome, un chiffrage complet natif, un P5 hors ligne, un cache documentaire intégral implicite ou une décision automatique.

## 1. Navigation et formats

Premier niveau — **FIGÉ** : **Accueil / Opportunités / Affaires / Entreprise / Administration**. Administration est groupée à part dans la navigation, selon droits, mais demeure un espace identifiable. Recherche, activité, traitements et profil sont des utilitaires de C00.

Dans l’Affaire — **FIGÉ pour prototypage** : **Vue d’ensemble / Documents / Prix / Réponse / Remise**, selon accès. Décision, Partenaires, Résultat/passation sont des destinations contextuelles accessibles par des actions explicites et par le menu Affaire. Visites et Questions acheteur ont des vues nommées depuis la synthèse. Le contexte entreprise candidate/lot/phase/tour demeure visible.

Formats adoptés pour le prototypage : **vue** = espace de travail avec lien direct ; **onglet** = vue nommée du même espace ; **section** = contenu dans sa page ; **panneau** = inspection/action courte avec retour ; **parcours** = séquence guidée reprenable ; **état** = variante avec action de reprise ; **mode large** = tâche longue en pleine largeur ; **mobile** = variante ciblée. Un panneau peut s’ouvrir en mode large sans créer une nouvelle destination de menu.
...
## 3. Les 16 espaces de travail canoniques — ADOPTÉS
...
### Détails structurants des fusions — ADOPTÉS POUR PROTOTYPAGE
...
## 5. Contrats transversaux N01–N05 — ADOPTÉS
...
Ces cinq contrats sont **adoptés**. Ils individualisent des besoins déjà évoqués dans le v0.1/v0.4 et complètent l’inventaire testable sans étendre les pouvoirs ni le périmètre commercial. N02 détaille particulièrement une interaction insuffisamment dessinée : une opération peut avoir réussi côté service sans que l’utilisateur en ait reçu la confirmation.
...
## 6. Parcours propriétaire : PUX-01–PUX-19 — ADOPTÉS

La matrice ci-dessous fixe les rattachements propriétaires de cette version. PUX-19 est désormais adopté au même titre que PUX-01–18.
...
Les recettes ci-dessous sont **rattachées, non exécutées**. Leur texte attendu reste celui du v0.4. Pour G18, la règle propriétaire applicable est définitivement A02/OWN-01 amendée : la suspension du dernier Propriétaire compromis est autorisée et la récupération reste encadrée. Aucun nouvel arbitrage n’est requis.
...
## 8. Ordre propriétaire de prototypage : parcours complets, puis couverture totale
...
## 8.1 Innovations UX retenues pour prototypage

Ces douze concepts issus de la contre-revue sont **adoptés comme hypothèses UX V1 à prototyper dans les contrats existants**. Ils ne sont pas présentés comme innovations uniques au marché et n’étendent pas le périmètre métier.

| ID | Concept retenu | Intégration propriétaire | Garde-fou |
|---|---|---|---|
| **I01** | **La minute Patron** | C01/C07 : bref par décision avec changement, portée, inconnus, condition, autorité et preuve immédiate. | Aucun résumé ne masque B0/B1 ni ne classe par score opaque. |
| **I02** | **À résoudre, sans doublons** | C05 : une file opérationnelle d’objets typés reliés aux tâches et impacts. | Une tâche faite ne ferme pas l’objet métier sans revue/preuve. |
| **I03** | **Diff de décision** | C06/C07 : après rectificatif, comparer ce qui fondait la décision précédente à l’état actuel et lister les validations dépendantes. | Impact confirmé ≠ inférence IA ; préserver l’historique. |
| **I04** | **Facture de l’inconnu** | C08 : rendre visibles besoins non couverts, devis expirés et hypothèses ; chiffrer seulement si estimation sourcée. | Inconnu ≠ zéro ; éviter double compte et fausse précision. |
| **I05** | **Question prête à envoyer** | C05 : depuis une contradiction, préparer un brouillon neutre avec sources et impact attendu. | Aucun envoi autonome ; stratégie sensible protégée. |
| **I06** | **Passeport de réemploi** | C13/C10 : source, version, validité, portée, applicabilité et droits de réemploi de l’élément utilisé. | Badge unique interdit comme preuve d’applicabilité. |
| **I07** | **Bon paquet, bonne preuve** | C11 : afficher en permanence candidate / autorisation / transmission / réception. | Ne jamais conclure « tout remis » depuis un reçu partiel. |
| **I08** | **Promesse transmise au chantier** | C10/C12 : relier engagement, moyen/coût, responsable futur et réserve de prise en charge. | Ne pas dériver vers ERP chantier. |
| **I09** | **Reprise à J−2** | N04/C05/C14 : relève nominative des conditions, validations, pièces, questions et tâches orphelines. | Aucun transfert automatique de délégation ou P5. |
| **I10** | **Ce qui a changé pour moi** | C01/C05 : vue depuis la dernière revue confirmée, limitée aux objets autorisés demandant une action. | Un problème ancien non résolu reste visible. |
| **I11** | **Préparer ma visite** | C05/N05 : pièces réellement disponibles, obligation de présence, preuves attendues, captures en attente. | Pas de promesse de dossier intégral hors connexion. |
| **I12** | **Une revue Entreprise qui débloque du travail** | C13 : montrer les usages autorisés affectés par une preuve à revoir et prioriser les revues utiles. | Aucun compteur révélant des Affaires non autorisées. |

Le **préchargement intégral d’un dossier hors connexion** reste hors décision V1. N05 décrit seulement la disponibilité réelle des éléments et l’état local/en attente/confirmé.

## 9. Changelog propriétaire v0.3
...
## 10. Conditions de validation avant UX Freeze

Les huit C1 sont désormais arbitrés et fermés au niveau conception. Avant UX Freeze : matérialiser toutes les surfaces et variantes applicables, exécuter les parcours PUX-01–19 et G01–G52, vérifier l’absence de fuite/pouvoir induit, corriger les défauts et recueillir la validation propriétaire. Les points W juridiques et de service restent ceux du v0.4 ; ce catalogue n’en annonce aucune clôture.
...
## 11. Registre propriétaire de statut v0.3

| Élément | Statut |
|---|---|
| Catalogue v0.1 | **SUPERSEDÉ pour l’organisation UX ; conservé comme origine des 104 contrats** |
| Work Review v0.2 | **ABSORBÉ comme matière d’audit ; non normatif seul** |
| 104 IDs | **CONSERVÉS — contrats de couverture** |
| C00 | **ADOPTÉ** |
| C01–C16 | **ADOPTÉS — espaces canoniques de conception** |
| N01–N05 | **ADOPTÉS — contrats transversaux** |
| PUX-01–PUX-19 | **ADOPTÉS — parcours de validation** |
| UX-C1-01 à UX-C1-08 | **FERMÉS AU NIVEAU CONCEPTION** |
| UX-C2 | **À éprouver/corriger avant UX Freeze** |
| UX-C3 | **Améliorations intégrées lorsque peu coûteuses ; non bloquantes seules** |
| G01–G52 | **RECETTES À MATÉRIALISER / EXÉCUTER** |
| I01–I12 | **RETENUES POUR PROTOTYPAGE V1** |
| UX Freeze | **NON ATTEINT** |
| Product Freeze v1.0 | **NON ATTEINT** |
| A2 SourceAnchor | **HOLD** |

## 12. Prochaine opération

La prochaine opération n’est plus une nouvelle contre-revue générale.

Elle est la **production des prototypes selon V0 puis V1**, en commençant par :
- C00 et le shell ;
- la navigation permanente et le contexte d’Affaire ;
- C05 « À résoudre » ;
- C11 Remise et ses quatre états ;
- confidentialité économique ;
- N01 conflit de contributions ;
- N02 issue d’opération inconnue ;
- puis l’entrée réelle C15 → C01 → C03/C06/C05 avec droits, panne et reprise.

Toute divergence découverte dans les prototypes est enregistrée comme écart UX versionné. Elle ne modifie pas silencieusement le Cahier directeur Produit & Métier v0.4.

---

**Fin — Owner Consolidated UX v0.3**
