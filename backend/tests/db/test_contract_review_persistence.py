...
def test_postgres_keeps_revision_2_append_only_and_rejects_duplicate_review(
    ...
    with Session(database_engine) as session:
        session.add(TenantRecord(id=tenant_id, slug=f"review-{tenant_id.hex[:12]}", lifecycle="ACTIVE"))
        session.flush()
        session.add(_case(tenant_id=tenant_id, case_id=case_id, marker="r"))
        session.flush()
        for revision in (1, 2):
            session.add(ContractBaselineDeviationImpactRecord(
                id=uuid4(), tenant_id=tenant_id, case_id=case_id,
                baseline_observation_id=observation_id, proof_revision=revision,
                baseline_source_refs_json=["dce://ccap/p4"], baseline_statement="Baseline",
                deviation_statement=None, impact_statement=None, status="UNKNOWN",
            ))
        session.flush()
        review = ContractProofReviewRecord(
            id=uuid4(), tenant_id=tenant_id,
            proof_id=session.scalar(sa.select(ContractBaselineDeviationImpactRecord.id).where(
                ContractBaselineDeviationImpactRecord.tenant_id == tenant_id,
                ContractBaselineDeviationImpactRecord.proof_revision == 2,
            )), reviewer_id=uuid4(), reviewed_revision=2,
            decision="NEEDS_CLARIFICATION", rationale="Révision requise",
        )
        session.add(review)
        session.commit()
        session.add(ContractProofReviewRecord(
            id=uuid4(), tenant_id=tenant_id, proof_id=review.proof_id,
            reviewer_id=uuid4(), reviewed_revision=2,
            decision="NEEDS_CLARIFICATION", rationale="Rejeu",
        ))
