...
def base():
    return {"command_id": uuid4(), "idempotency_key": uuid4(), "correlation_id": uuid4(), "outcome_id": uuid4(), "case_id": uuid4(), "lot_reference": "01"}

def test_outcome_contract_distinguishes_won_and_unknown_evidence():
    won = RecordCaseOutcomeCommand(**base(), outcome="WON", source_locator="portal:receipt:1", reservations=["Sous réserve de validation du planning"])
    unknown = RecordCaseOutcomeCommand(**base(), outcome="UNKNOWN", unknown_reason="Aucun retour contrôlé")
