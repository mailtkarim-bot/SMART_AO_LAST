...
def _client(*, service=None, resolver_error=None):
    ...
    app.include_router(
        build_collaborator_info_blocker_router(
            service=service or cast(Any, _WorkflowService()),
...
def _request_payload():
    return {
        ...
        "subject": "Source manquante",
        "question": "Quelle pièce doit être fournie ?",
        "requested_object": "Attestation",
        "reason": "Le contrôle ne peut pas être terminé.",
        "priority": "HIGH",
        "due_at": None,
    }
...
def _response_payload():
    return {
        ...
        "source_locator": "document:attestation:1",
        "outcome": "ANSWERED",
    }
...
def _blocker_payload():
    return {
        ...
        "description": "Pièce obligatoire absente.",
        "source_locator": "requirement:12",
...
class _WorkflowService:
    ...
    def read_workflow(self, **kwargs):
        ...
        request = SimpleNamespace(
            ...
            request_kind="MISSING_SOURCE",
            subject="Source manquante",
            question="Quelle pièce ?",
            requested_object="Attestation",
            reason="Contrôle bloqué.",
            priority="HIGH",
            state="ANSWERED",
            due_at=None,
            aggregate_revision=2,
...
def test_workflow_commands_cover_create_answer_block_and_resolve():
    service = cast(Any, _WorkflowService())
    ...
    paths = [
        ...
        (
            f"/api/v1/collaborator/information-requests/{request_id}/responses",
            _response_payload(),
            "INFORMATION_REQUEST_ANSWERED",
        ),
        (f"/api/v1/collaborator/tasks/{task_id}/blockers", _blocker_payload(), "TASK_BLOCKED"),
        (
            f"/api/v1/collaborator/tasks/{task_id}/blockers/{uuid4()}/resolve",
            _resolve_payload(),
            "TASK_UNBLOCKED",
        ),
    ]
...
def test_workflow_payload_rejects_financial_fields():
    response = _client().post(
        f"/api/v1/collaborator/tasks/{uuid4()}/blockers",
        json={**_blocker_payload(), "gross_margin_minor": 100},
        headers=_headers(),
    )
