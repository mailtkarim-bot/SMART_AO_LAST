...
class TotpFactorRecord(Base):
    """Encrypted TOTP factor owned by one global identity."""

    __tablename__ = "mfa_totp_factors"
    __table_args__ = (
        sa.ForeignKeyConstraint(
...
class TotpRecoveryCodeRecord(Base):
    """One-time hashed recovery code for a TOTP factor."""

    __tablename__ = "mfa_totp_recovery_codes"
    __table_args__ = (
        sa.ForeignKeyConstraint(
            ["factor_id"],
            ["mfa_totp_factors.id"],
