...
from app.modules.dce.infrastructure.models.export_proof_verification import (
...
class ExportVerificationTimelineReadService:
    ...
    def get_for_export(self, *, actor, export_id):
        ...
        with self._session_factory() as session:
            rows = []
            for row in session.scalars(sa.select(ExportProofVerificationRecord).where(ExportProofVerificationRecord.tenant_id == actor.tenant_id, ExportProofVerificationRecord.export_id == export_id)).all(): rows.append({"event_type": "VERIFICATION", "event_id": row.id, "status": row.outcome, "created_at": row.created_at})
            ...
            for row in session.scalars(sa.select(ExportVerificationOwnerActRecord).where(ExportVerificationOwnerActRecord.tenant_id == actor.tenant_id, ExportVerificationOwnerActRecord.export_id == export_id)).all(): rows.append({"event_type": "OWNER_ACT", "event_id": row.id, "status": "APPROVED" if row.approved else "REJECTED", "created_at": row.created_at})
